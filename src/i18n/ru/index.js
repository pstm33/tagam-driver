import en from "../en";

export default {
  ...en,
  "Let's Sign You In": "Вход для курьера",
  "Enter your username and password to sign in":
    "Введите логин и пароль, чтобы войти",
  "Enter your username and password to sigin":
    "Введите логин и пароль, чтобы войти",
  "Email address": "Email",
  "This field is required": "Обязательное поле",
  Password: "Пароль",
  "Forgot Password": "Забыли пароль",
  "Sign In": "Войти",
  "Don't have an account": "Нет аккаунта",
  Signup: "Регистрация",
  Submit: "Отправить",
  Continue: "Продолжить",
  Cancel: "Отмена",
  Save: "Сохранить",
  Update: "Обновить",
  Delete: "Удалить",
  Close: "Закрыть",
  Done: "Готово",
  OK: "OK",
  Home: "Главная",
  Schedule: "График",
  "My Schedule": "Мой график",
  Account: "Аккаунт",
  Legal: "Правовая информация",
  Logout: "Выйти",
  "Sign Out": "Выход",
  "Do you want to logout?": "Выйти из аккаунта?",
  "Log out": "Выйти",
  Language: "Язык",
  "Select Language": "Выбор языка",
  Skip: "Пропустить",
  Settings: "Настройки",
  Status: "Статус",
  Availability: "Доступность",
  Online: "Онлайн",
  Offline: "Офлайн",
  Working: "В работе",
  "Not Working": "Не в работе",
  Location: "Локация",
  Enabled: "Включена",
  Disabled: "Выключена",
  Balance: "Баланс",
  Deliveries: "Заказы",
  Orders: "Заказы",
  Maps: "Карта",
  Map: "Карта",
  Chat: "Чат",
  Call: "Позвонить",
  Route: "Маршрут",
  Pins: "Метки",
  History: "История",
  Wallet: "Кошелек",
  Earnings: "Заработок",
  Shifts: "Смены",
  Notifications: "Уведомления",
  "Today Schedule": "Расписание на сегодня",
  "Manage your shift, location and current delivery status":
    "Управляйте сменой, локацией и текущими заказами",
  "Your location is disabled": "Геолокация выключена",
  "Enabled Location": "Включить геолокацию",
  "Enabled Your Location": "Включите геолокацию",
  "In order to continue we need your location enabled":
    "Чтобы продолжить, приложению нужна геолокация",
  "Skip For Now": "Пока пропустить",
  "Go online": "Выйти на линию",
  "Go offline": "Уйти с линии",
  "You have no work schedule for today": "Сегодня нет рабочей смены",
  "View Car Documents": "Документы автомобиля",
  "Car Documents": "Документы автомобиля",
  "Starting in": "Начало через",
  Ongoing: "Идет сейчас",
  "Are you ready": "Готовы начать",
  "Start Working Now": "Начать работу",
  "Request Break": "Запросить перерыв",
  "End Break": "Завершить перерыв",
  "End shift": "Завершить смену",
  "Search Shifts": "Найти смены",
  "Available shift": "Доступная смена",
  "No available shift": "Нет доступных смен",
  "Take Shift": "Взять смену",
  "Current shift": "Текущая смена",
  "shift ended": "Смена завершена",
  Date: "Дата",
  Area: "Район",
  Time: "Время",
  "Pull down the page to refresh": "Потяните вниз, чтобы обновить",
  "You have no pending deliveries": "Нет ожидающих заказов",
  "Make sure your internet is always connected":
    "Проверьте, что интернет подключен",
  "No available data": "Нет данных",
  "Destination is ready": "Точка назначения готова",
  "No active delivery destination": "Нет активной точки доставки",
  "Order#": "Заказ #",
  "New Order #": "Новый заказ #",
  "Delivery Details": "Детали доставки",
  Pickup: "Забрать",
  "Drop-off": "Доставить",
  "Accepting Orders": "Принятие заказа",
  "Release to accept": "Отпустите, чтобы принять",
  "Slide right to accept": "Проведите вправо, чтобы принять",
  "Drag the handle": "Потяните бегунок",
  Accept: "Принять",
  Decline: "Отклонить",
  "Are you sure you want to decline this order":
    "Вы уверены, что хотите отклонить заказ",
  "Tell us why": "Укажите причину",
  "Please specify the reason": "Пожалуйста, укажите причину",
  Others: "Другое",
  "Restaurant Details": "Ресторан",
  "Restaurant time preparation": "Время приготовления",
  "Estimated Arrival": "Расчетное прибытие",
  "Customer Details": "Клиент",
  Payment: "Оплата",
  "Drop-off instructions": "Инструкции доставки",
  "Delivery instructions": "Инструкции для курьера",
  "Delivery options": "Параметры доставки",
  "Aparment, suite or floor": "Квартира, офис или этаж",
  "Paid online": "Оплачено онлайн",
  "Collect Cash": "Получить наличные",
  Items: "Позиции",
  Item: "Позиция",
  "Order Details": "Состав заказа",
  "Confirm Drop-off": "Подтвердить доставку",
  "Total Payment": "Итого к оплате",
  "Collect cash from customer": "Получите наличные у клиента",
  "Proof of Drop-off": "Фото подтверждения доставки",
  "Available balance": "Доступный баланс",
  "CASH IN": "ПОПОЛНИТЬ",
  "Cash In": "Пополнение",
  "Cash In Amount": "Сумма пополнения",
  "Wallet transactions": "Операции по кошельку",
  "Add to your balance": "Пополнить баланс",
  "how much do you want to add to your account":
    "какую сумму добавить на счет",
  "Enter top up amount": "Введите сумму пополнения",
  "Transaction history": "История операций",
  "Total delivered": "Всего доставлено",
  "Cash collected": "Получено наличными",
  "Delivery pay": "Оплата доставок",
  "Total Tips": "Чаевые",
  "Delivery ledger": "Журнал доставок",
  "Completed orders, collected cash and delivery pay by date":
    "Выполненные заказы, собранные наличные и оплата доставок по датам",
  "Account Settings": "Настройки аккаунта",
  "Courier profile": "Профиль курьера",
  "Update contact details used by dispatch and support":
    "Обновите контактные данные для диспетчера и поддержки",
  Security: "Безопасность",
  "Update courier account credentials": "Обновите данные входа курьера",
  "Payout bank": "Банк для выплат",
  "Review and update payout bank details":
    "Проверьте и обновите банковские данные для выплат",
  "Keep courier identity and license data ready for dispatch approval":
    "Держите данные курьера и водительского удостоверения готовыми для проверки",
  "Courier transport": "Транспорт курьера",
  "Vehicle details used for shift assignment and dispatch identity":
    "Данные транспорта используются для смен и идентификации курьера",
  "Payment methods": "Способы оплаты",
  "Payment method": "Способ оплаты",
  "Choose a provider to add a saved payment method":
    "Выберите провайдера, чтобы добавить сохраненный способ оплаты",
  "Saved cards and wallets for courier balance top-ups":
    "Сохраненные карты и кошельки для пополнения баланса",
  Inbox: "Входящие",
  "Dispatch messages, account updates and delivery alerts":
    "Сообщения диспетчера, обновления аккаунта и уведомления о доставках",
  "Dispatch conversations and order support":
    "Диалоги с диспетчером и поддержка заказов",
  "Conversations appear when dispatch, merchant or customer support opens a thread":
    "Диалоги появятся, когда диспетчер, ресторан или поддержка клиента откроет чат",
  "Shift records": "История смен",
  "Past and assigned shift windows, vehicles and delivery counts":
    "Прошедшие и назначенные смены, транспорт и количество доставок",
  "Available balance top-up method": "Способ пополнения доступного баланса",
  "No saved payments": "Нет сохраненных способов оплаты",
  "Add a card or wallet to top up faster":
    "Добавьте карту или кошелек для быстрого пополнения",
  "Browser location can be blocked during local testing. Skipping keeps the courier dashboard available; mobile tracking will still request GPS on the real app":
    "В браузере геолокация может быть заблокирована при локальном тестировании. Пропуск оставит панель курьера доступной, а в мобильном приложении GPS будет запрошен отдельно",
  "Manage Profile": "Профиль",
  "Change Password": "Сменить пароль",
  "Change password": "Смена пароля",
  Documents: "Документы",
  Vehicle: "Транспорт",
  "Bank information": "Банковские данные",
  "Payment Options": "Способы оплаты",
  "Push Notifications": "Push-уведомления",
  "Keep Awake": "Не гасить экран",
  "Dark Mode": "Темная тема",
  Direction: "Направление",
  "Delete Account": "Удалить аккаунт",
  "Edit profile": "Редактировать профиль",
  Edit: "Изменить",
  "First name": "Имя",
  "Last name": "Фамилия",
  "Old password": "Старый пароль",
  "New password": "Новый пароль",
  "Confirm password": "Подтвердите пароль",
  "Update Photo": "Обновить фото",
  "Driving License": "Водительское удостоверение",
  License: "Права",
  "Driver's License": "Водительское удостоверение",
  "We need your driving license information":
    "Нужны данные водительского удостоверения",
  "Enter your license correctly": "Введите данные удостоверения правильно",
  "Make sure the entire license and all the details are VISIBLE":
    "Убедитесь, что удостоверение и все данные полностью видны",
  "Front Photo of ID": "Фото лицевой стороны",
  "Back Photo of ID": "Фото обратной стороны",
  "Maximum 2MB. Accepted types: PNG. JPG":
    "Максимум 2 МБ. Форматы: PNG, JPG",
  "License number": "Номер удостоверения",
  "Expiration date": "Срок действия",
  "Take a Photo": "Сделать фото",
  "Upload a Photo": "Загрузить фото",
  "Confirm Photo": "Подтвердить фото",
  "Change Photo": "Изменить фото",
  "Add Photo": "Добавить фото",
  "Vehicle Photo": "Фото транспорта",
  "Vehicle type": "Тип транспорта",
  "Plate number": "Номерной знак",
  Maker: "Марка",
  Model: "Модель",
  Color: "Цвет",
  "Bank account": "Банковский счет",
  "Bank account details": "Данные банковского счета",
  "Please enter correct bank details to avoid delayed payment":
    "Введите корректные банковские данные, чтобы избежать задержки выплаты",
  "Bank account holders name": "Владелец счета",
  "Bank account number/IBAN": "Номер счета / IBAN",
  "Swift Code": "SWIFT-код",
  "Bank name in full": "Название банка",
  "Bank branch city": "Город отделения",
  "Bank branch": "Отделение банка",
  Payments: "Платежи",
  "No Payment available": "Нет способов оплаты",
  "you have not added payment yet": "Вы еще не добавили способ оплаты",
  "Add new payment": "Добавить способ оплаты",
  "Add payment": "Добавить оплату",
  "Delete account": "Удаление аккаунта",
  "Confirm Deletion": "Подтвердить удаление",
  "Total earnings": "Всего заработано",
  "View details": "Подробнее",
  "Cash out": "Вывод средств",
  Sale: "Продажи",
  "Total Trips": "Всего поездок",
  Daily: "День",
  Weekly: "Неделя",
  Tips: "Чаевые",
  Incentives: "Бонусы",
  Adjustment: "Корректировка",
  "View earnings activity": "История заработка",
  "Earnings Activity": "Активность заработка",
  "Earnings activity": "Активность заработка",
  "Earnings detail": "Детали заработка",
  "Order-level earnings, tips and delivery pay":
    "Заработок по заказам, чаевые и оплата доставки",
  "Cash out history": "История выводов",
  Cashout: "Вывод",
  "Processing fee": "Комиссия",
  "Add Bank Account": "Добавить банковский счет",
  "You receive": "Вы получите",
  "Confirm Transfer": "Подтвердить перевод",
  "Take photo": "Сделать фото",
  "Proof of Car": "Фото транспорта",
  "Add photo": "Добавить фото",
  "Delete All": "Удалить все",
  "Driver Overview": "Обзор курьера",
  "Total reviews": "Всего отзывов",
  "Filter by zone": "Фильтр по зоне",
  Clear: "Очистить",
  Apply: "Применить",
  Help: "Помощь",
  Support: "Поддержка",
  "Call Support": "Позвонить в поддержку",
  "Chat Support": "Чат поддержки",
  "Live Chat": "Чат",
  "Chat will show here": "Здесь будет чат",
  "New Message": "Новое сообщение",
  "View Message": "Открыть сообщение",
  You: "Вы",
  "Your message": "Ваше сообщение",
  "Please wait": "Пожалуйста, подождите",
  Okay: "Хорошо",
  No: "Нет",
  "Network Error": "Ошибка сети",
  "We're having trouble loading": "Не удалось загрузить данные",
  "Please check your Network connectivity and try again":
    "Проверьте подключение к интернету и попробуйте снова",
  "Try Again": "Повторить",
  "No internet connection": "Нет подключения к интернету",
  "Press BACK again to exit": "Нажмите Назад еще раз для выхода",
  "Driver app": "Приложение курьера",
  "KMRS courier module": "Курьерский модуль KMRS",
  "TAGAM courier workspace": "Рабочее приложение курьера TAGAM",
  "Orders shifts map and wallet in one clear interface over KMRS boxed logic":
    "Заказы, смены, карта и кошелек в одном понятном интерфейсе поверх коробочной логики KMRS",
  Tracking: "Трекинг",
  Mode: "Режим",
  Role: "Роль",
  Ready: "Готов",
  Driver: "Курьер",
  driver: "курьер",
  contractor: "подрядчик",
  employee: "сотрудник",
  Policies: "Документы",
  "Terms privacy and operating documents": "Условия, приватность и рабочие документы",
  "Courier help desk and operational contact points":
    "Поддержка курьера и рабочие контакты",
  "Email support": "Поддержка по email",
  "Support channel": "Канал поддержки",
  All: "Все",
  "Courier onboarding": "Регистрация курьера",
  "Live support conversation": "Диалог с поддержкой",
  Total: "Итого",
  "Order total": "Итого заказа",
  Viewing: "Просмотр",
  "Finish active schedule": "Завершить активную смену",
  "View order": "Открыть состав",
  "View order items": "Состав заказа ({count})",
  "New assignment": "Новое назначение",
  Destination: "Клиент",
  Pickup: "Получение",
  "Drop-off": "Доставка клиенту",
  "Mark as delivered": "Отметить как доставлено",
  "Confirm delivery": "Подтвердить доставку",
  "Confirm Pickup": "Подтвердить получение",
  "Payment change": "Сдача",
  "Do you confirm order#": "Подтвердить действие по заказу #",
  "Report an issue": "Сообщить о проблеме",
  "Need help": "Нужна помощь",
  "Call Support": "Позвонить в поддержку",
  "Chat Support": "Чат поддержки",
  "What is the issue": "Что случилось",
  "Go to restaurant": "Ехать в ресторан",
  "Go to merchant": "Ехать в ресторан",
  "Go to customer": "Ехать к клиенту",
  "On the way to merchant": "В пути к ресторану",
  "On the way to customer": "В пути к клиенту",
  "Arrived at merchant": "Прибыл в ресторан",
  "Arrived at vendor": "Прибыл в ресторан",
  "Arrived at customer": "Прибыл к клиенту",
  "Waiting for order": "Ожидание заказа",
  "Order pickup": "Заказ получен",
  "Unable to find address": "Не получается найти адрес",
  "Order was cancelled": "Заказ отменен",
  "Food spill": "Проблема с заказом",
  "Customer can’t be reached": "Не удается связаться с клиентом",
  "Customer can't be reached": "Не удается связаться с клиентом",
  "Dropoff address is changed": "Адрес доставки изменился",
  "Dropoff is inaccessible": "Нет доступа к месту доставки",
  "No earnings for this period": "Нет заработка за этот период",
  "Completed deliveries will appear here": "Завершенные доставки появятся здесь",
  "{amount} is the max you can cash out each day":
    "Максимум для вывода в день: {amount}",
  "Select Zone": "Выбор зоны",
  "Dispatch area": "Зона доставки",
  "No zones available": "Нет доступных зон",
  "Create or assign zones in KMRS backoffice for this merchant":
    "Создайте или назначьте зоны в админке KMRS для этого ресторана",
  "Before you start receiving orders, please choose your preferred zone. Select a zone to get started and receive orders from your desired location":
    "Перед получением заказов выберите рабочую зону. Заказы будут приходить из выбранной зоны",
};
