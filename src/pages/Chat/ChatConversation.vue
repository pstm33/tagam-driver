<template>
  ...
  <q-page class="flex items-stretch content-end bg-[#f7f7f5] px-4 py-5">
    <div class="w-full max-w-[520px]">
      ...
      <template v-if="!conversation_found">
        <div class="rounded-[8px] border border-[#ece7dd] bg-white p-5 text-center text-[18px] font-black text-tagam-ink">
      ...
      <template v-for="items in getChatmessage" :key="items">
        <template v-if="items.messageType == 'audio'">
          <q-chat-message
            ...
            :bg-color="items.senderID == user_uuid ? 'amber-8' : 'grey-2'"
            ...
            <template #avatar>
              <q-avatar class="ml-2">
                <q-img
                  :src="items.photo"
                  spinner-size="sm"
                  spinner-color="amber-6"
                  class="h-12 min-w-[48px] max-w-[48px]"
                  fit="cover"
                  loading="lazy"
                ></q-img>
              </q-avatar>
          ...
          >
            ...
            <template v-if="items.fileUrl">
              <q-img
                ...
                spinner-size="sm"
                spinner-color="amber-6"
                class="min-h-[150px] min-w-[150px] max-w-[150px] rounded-[8px]"
              >
              </q-img>
            </template>
          </q-chat-message>
      ...
      <template v-for="(items, userUUID) in getUserTyping" :key="items">
        <template v-if="items">
          <template v-if="userUUID != user_uuid">
            <template v-if="from_info">
              <q-chat-message
                :name="`${from_info.first_name} ${this.$t('is typing...')}`"
                :avatar="from_info.photo"
    ...
    <div ref="scroll_ref" class="p-2 text-white"></div>
  </q-page>
  <q-footer
    v-if="conversation_found"
    class="border-t border-[#ece7dd] bg-white px-3 py-2 text-tagam-ink"
  >
          ...
          <div class="flex justify-start gap-3 py-2">
            <template v-for="file in scope.files" :key="file.__key">
              <div class="relative-position">
                <img
                  :src="file.__img.src"
                  class="h-[60px] max-w-[60px] rounded-[8px] object-cover"
                />
                <div class="absolute-right -mr-2 -mt-1">
                  <q-btn
                    unelevated
                    round
                    color="amber-6"
...
export default {
  ...
  created() {
    this.chat_api_url = this.chat_api_url.replace("/driver/", "/");
  ...
  mounted() {
    ...
    this.user_uuid = this.user_data.driver_uuid;
    this.getMessages();
    this.getParticipant();
    this.getWhoIsTyping();
  },
  ...
  methods: {
    ...
    notifyUser() {
      ...
      params += "&first_name=" + this.to_info.first_name;
      params += "&last_name=" + this.to_info.last_name;
      axios
    ...
    async UploadFile() {
      try {
        ...
        const response = await api.post(
          `${config.api_base_url}/uploadaudio`,
          data,
          {
            headers: {
